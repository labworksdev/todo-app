...
- Type hints required on all function signatures; Google-style docstrings on all public APIs
- Use `pathlib.Path` for file operations; prefer context managers (`with` blocks)
- Logging: `logger = logging.getLogger(__name__)`; log outcomes, never sensitive values
